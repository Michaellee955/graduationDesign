		        ...
		        <label class="control-label labelCss" for="productId">需求产品：</label>
		         <select id="serialId" name="serialId" class="input-sm" >
			        	...
			        	<c:forEach items="${cvList}" var="cv">
			        		<option value="${cv.fkey}" <c:out value="${sr.serialId==cv.fkey?'selected':'' }"/>>${cv.fvalue}</option>
			        	</c:forEach>
			        </select>
			...
			<div class="form-group">
		        <label class="control-label labelCss" for="serviceType">需求类型：</label>
		         <select id="sr_serviceType" name="serviceType" class="input-sm">
		        	<option value="1" <c:out value="${sr.serviceType==1?'selected':'' }"/>>设计</option>
		        	<option value="2" <c:out value="${sr.serviceType==2?'selected':'' }"/>>制造</option>
		        	<option value="3" <c:out value="${sr.serviceType==3?'selected':'' }"/>>测试</option>
		        	<option value="4" <c:out value="${sr.serviceType==4?'selected':'' }"/>>仿真</option>
		        	<option value="5" <c:out value="${sr.serviceType==5?'selected':'' }"/>>软件</option>
		        </select>
			</div>
			<div class="form-group">
		     ...
		     <div class="form-group">
		        <label class="control-label labelCss" for="requireUint">需求单位：</label>
		         <select id="sr_requireUint" name="requireUint" class="input-sm">
		        	<option value="4" <c:out value="${sr.requireUint==4?'selected':'' }"/>>个</option>
		        </select>
		       	<span style="color: red">*</span>
			</div>
			<div class="form-group">
		        <label class="control-label labelCss" for="requireQualify">需求质量：</label>
		          <select id="sr_requireQualify" name="requireQualify" class="input-sm">
		        	<option value="1" <c:out value="${sr.requireQualify==1?'selected':'' }"/>>GE认证</option>
		        	<option value="2" <c:out value="${sr.requireQualify==2?'selected':'' }"/>>3C认证</option>
		        	<option value="3" <c:out value="${sr.requireQualify==3?'selected':'' }"/>>rosh认证</option>
		        </select>
			</div>
			<div class="form-group">
...
$(document).ready(function(){
	$("#currPos").html("服务需求<label class='icon-double-angle-right'/>需求发布");
		...
		if( '${sr.id}'){
			generPsTree('${sr.serialId}',function(){
				$("#structTreeDiv").hide();
			});
		}
});
